...
function resizeCanvases() {
  ...
  dom.canvasInfo.textContent = `画布：${targetW}×${targetH} @${dpr} DPR`;
}

function updateVideoInfo() {
  const fpsHint = useFileSource ? "本地源" : `${sources[dom.sourceSelect.value]?.fps || "--"}fps`;
  const info = video.videoWidth
    ? `视频：${video.videoWidth}×${video.videoHeight} ｜ ~${fpsHint}`
    : "视频：-- × --";
...
function updateSRGBStatus() {
  ...
  if (supported) {
    dom.srgbInfo.textContent = state.useSRGB ? "sRGB：已开启（gamma 2.2）" : "sRGB：可用";
  } else {
    dom.srgbInfo.textContent = "sRGB：不支持 EXT_sRGB";
  }
}
...
function attachEvents() {
  ...
  video.addEventListener("play", () => {
    dom.playToggle.textContent = "暂停";
    startRenderLoop();
  });
  video.addEventListener("pause", () => {
    dom.playToggle.textContent = "播放";
    stopRenderLoop();
  });
}
...
function boot() {
  ...
  attachEvents();
  updateSRGBStatus();
  dom.videoInfo.textContent = "视频：加载中…";
  loadSelectedSource();
}
