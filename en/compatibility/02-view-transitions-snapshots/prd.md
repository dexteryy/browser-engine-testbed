...
## R&D side⽤example 02 ｜ View Transitions Composition and snapshots of cross-state transitions

> most⼩ JS Dependency: make⽤ View Transitions API Trigger cross-state snapshots and transitions.

### Test target

Evaluate View Transitions ⽣old state/New snapshot in GPU Cost, number of composition layers and memory usage in compositing⽤;Verify across
⻚⾯(or across routes) with the same⻚Differences in state switching.

### Features involved

View Transitions(document.startViewTransition), view-transition-name Name optional elements; over
Watauchi Tsune⽤of opacity/transform animation.

### Page structure and key points

- Two states:S1(List of dense thumbnails⽹case) and S2(⼤Figure details).
- Set the same settings for thumbnails and detail main images view-transition-name accomplish morph/cross light⼊fade out.
- ⼀Same version⻚status switching;⼀version to do cross routing (URL change) switch.

### Human observation and suggestion tools

- DevTools → Performance:Record the switching process, follow Snapshot capture, Raster, Composite.
- DevTools → Layers:Observe production during transition⽣ofNumber and size of snapshot layers⼨.
- Rendering → FPS meter:Record frame rate stability.

### Acceptance criteria

- Transitional visual coherence,⽆flashing/⿊screen/The levels are confused.
- Number of composition layers and memory usage⽤within the controllable range (⼤Graph snapshots should not persist for too long).
- across⻚/same⻚Both modes are stable 60fps(table⾯⾼match).

### Optional variants

- will transition⽬Change the icon from still image to filtered imageofpicture or⽂This block, observe extra render pass ofInfluence.
- ⼤Amount of thumbnails involved in the transitionofpress⼒scene.
