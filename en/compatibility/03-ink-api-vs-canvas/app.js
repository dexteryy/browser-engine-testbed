...
(() => {
  ...
  function clearAll() {
    ...
    leftHint.textContent = 'waiting for input...';
    rightHint.textContent = 'waiting for input...';
    state.lastRightArea = null;
    lastAreaEl.textContent = 'Redraw estimate on the right:--';
  }
  ...
  async function initInk() {
    ...
    if (hasInk) {
      try {
        ...
        inkSupportEl.textContent = 'Ink API:Enabled';
        ...
        inkSupportEl.textContent = 'Ink API:Initialization failed (downgraded)';
        inkSupportEl.style.background = '#fff3cd';
      }
    } else {
      inkSupportEl.textContent = 'Ink API:Not supported (downgraded)';
  ...
  function handleDrawEvents(e) {
    ...
    // UI hints
    rightHint.textContent = 'Drawing...';
    leftHint.textContent = state.ink.supported ? 'Hardware inking (synth overlay in progress)' : 'Drawing...';

    // Update status
    if (state.lastRightArea) {
      lastAreaEl.textContent = `Redraw estimate on the right:${state.lastRightArea.w}×${state.lastRightArea.h} Pixels (approximately)`;
    }
  }
  ...
  function pointerUpOrCancel(e) {
    ...
    leftHint.textContent = state.ink.supported ? 'Cured to canvas' : 'Finish';
    rightHint.textContent = 'Finish';
  ...
  useRaw.addEventListener('change', () => {
    eventStatusEl.textContent = 'event:' + (useRaw.checked ? 'pointerrawupdate' : 'pointermove');
  });
