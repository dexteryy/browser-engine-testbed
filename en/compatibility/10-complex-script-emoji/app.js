...
const arabicLines = [
  { label: "continuous writing+number", text: "الويب 2024: اختبارات مُحرّك المتصفح 12345 مع علامات ترقيم، سرعة وسلاسة.", bidiClass: "rtl" },
  { label: "Bidi shuffle", text: "RTL + LTR shuffle:ابتكار WebGL و Canvas في 2024، رقم ٣٫١٤ و (Hello!)", bidiClass: "bidi-mix" },
  { label: "paragraph", text: "مقارنة النص العربي مع الأرقام اللاتينية، وملاحظة اتجاه الفواصل والرموز: ؟ ! ، ؛", bidiClass: "rtl" }
];

const seaLines = [
  { label: "Thai", text: "ภาษาไทยทดสอบการผสมสระและวรรณยุกต์ เช่น เก็่ง กี่ ไก่ พร้อมตัวเลข ๒๕๖๗", className: "" },
  { label: "Vietnamese", text: "Tiếng Việt thử nghiệm dấu thanh: ắ ằ ẵ ẳ ặ, ệ ễ ể, cùng ký tự "đ" và số 123.", className: "" },
  { label: "Sanskrit/Attachment", text: "संस्कृत संयोजन चिह्न परीक्षणः देवनागरी के संयुक्ताक्षर एवं मात्रा चिह्न", className: "" }
];

const emojiSets = [
  { label: "family ZWJ", char: "👨‍👩‍👧‍👦", note: "family" },
  { label: "Multi-color handshake", char: "🧑🏾‍🤝‍🧑🏻", note: "handshake" },
  { label: "Nurse", char: "🧑‍⚕️", note: "health" },
  { label: "female astronaut", char: "👩🏽‍🚀", note: "astronaut" },
  { label: "male police officer", char: "👮🏻‍♂️", note: "police" },
  { label: "couple", char: "👩‍❤️‍💋‍👩", note: "kiss" },
  { label: "walk", char: "🚶🏿‍♂️", note: "walk" },
  { label: "running", char: "🏃‍♀️", note: "run" },
  { label: "🏳️‍🌈", char: "🏳️‍🌈", note: "flag" },
  { label: "cry with laughter", char: "😂", note: "face" },
  { label: "Wear a mask", char: "😷", note: "mask" },
  { label: "Rocket", char: "🚀", note: "rocket" }
];
...
function createGiantEmoji(column) {
  ...
  label.innerHTML = '<span>giant Emoji</span><span class="badge">256px</span>';
...
function renderColumns() {
  ...
  createSample("Arabparagraph", "RTL/Bidi", arabicLines, dom.arabicContent, false);
  createSample("Southeast Asian combinationAttachment", "Combining", seaLines, dom.seaContent, false);
  createSample("Emoji ZWJ sequence", "Emoji", [], dom.emojiContent, true);
  if (state.giantEmoji) {
...
function applyAnimations(options = {}) {
  ...
  const anyOn = state.animArabic || state.animSEA || state.animEmoji;
  dom.animStatus.textContent = `Animation: ${anyOn ? "on" : "all off"}`;
  if (anyOn) {
    dom.animStatus.textContent += ` · opacity ${state.fadeMin.toFixed(2)}→1 · cycle ${state.duration.toFixed(1)}s`;
  }
}
...
function applyFeatures() {
  ...
  dom.bidiStatus.textContent = state.bidiMarks ? "Bidi:control characters included" : "Bidi:Auto direction only";
