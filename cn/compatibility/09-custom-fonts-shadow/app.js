...
const samples = [
  {
    ...
    badge: "可变粗细/斜体",
    lines: [
      { text: "晴空流光 Typography 2024 — 自定义字体栅格化", size: "large" },
      { text: "Variable axis: Wght / Slnt — 滑块调节实时生效", size: "medium" }
    ]
  },
  {
    label: "正文混排与连字",
    badge: "liga/clig",
    lines: [
      { text: "快速的棕色狐狸跳过懒狗，锐利边缘对比阴影与滤镜。", size: "medium" },
  ...
  },
  {
    ...
    badge: "small-caps",
    lines: [
      { text: "Small Caps & 数字：0123456789，抗锯齿与 hinting 对照。", size: "small", smallcaps: true },
      { text: "微字号 12px：观察 subpixel 抗锯齿与描边融合。", size: "mini" }
    ]
  },
  {
    label: "衬线对照（静态 WOFF2）",
    badge: "serif",
    lines: [
      { text: "Serif Title — Source Serif 4 对照 text-shadow/drop-shadow。", size: "medium", serif: true },
      { text: "衬线正文：细节锐利度、字偶距、阴影模糊与背景融合。", size: "small", serif: true }
    ]
  },
  {
    label: "亚像素边缘对照",
...
function renderSamples(container, applyDrop) {
  ...
  samples.forEach(sample => {
    ...
    if (sample.subpixel) {
      ...
      ["RGB 次像素对照 AAAA", "垂直笔画 iiiiii / llll"].forEach(text => {
...
function measureFonts(label) {
  ...
  if (check) {
    ...
    dom.cacheStatus.textContent = `${label}：命中（check 约 ${cost}ms）`;
    return;
  }
  document.fonts.ready.then(() => {
    const cost = Math.round(performance.now() - start);
    dom.cacheStatus.textContent = `${label}：加载完成 ${cost}ms`;
  });
}
...
async function reloadFonts() {
  ...
  try {
    ...
    updateStatus("字体重新加载完成（Reload 家族可用）");
    measureFonts("Reload");
  } catch (err) {
    updateStatus("重新加载失败");
    console.error(err);
...
function boot() {
  ...
  attachEvents();
  document.fonts.ready.then(() => {
    updateStatus("字体已加载（Inter Var + Source Serif 4）");
    measureFonts("初始");
  });
  animateTicker();
  startScroll();
