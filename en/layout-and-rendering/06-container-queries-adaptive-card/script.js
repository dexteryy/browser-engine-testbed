  ...
  function bindSizeBadges(containers, getThreshold){
    const ro = new ResizeObserver(entries => {
      for(const entry of entries){
        ...
        badge.textContent = `container:${w}px / Threshold:${th}px`;
  ...
  function initStress(){
    ...
    const makeCard = (i) => {
      ...
      card.innerHTML = `<figure class="media">
        ...
        <figcaption>No. ${i+1} open</figcaption>
      </figure>
      <div class="body">
        <h3>card ${i+1}</h3>
        <p>Used for stress testing: quickly adjust the container width and observe the style switching overhead.</p>
        <ul class="meta"><li>serial number:${i+1}</li><li>Type: Test</li></ul>
        <button class="action">operate</button>
      </div>`;
      return card;
    };
